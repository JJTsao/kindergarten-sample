...
img = Image.open('SampleX.png'); print(img.size)  # 常是縮小的桌面版，用 1200px container 重建
```
...
- **裝飾**：設計稿在 hero/CTA 以外常很乾淨；若要更豐富，加**適度**spot 點綴（雲/星/葉/柔和 blob），墊在 `.deco-layer`（`position:absolute;inset:0;overflow:hidden;z-index:0`）下層、`.container{z-index:1}`，放留白角落、不擋內容；手機別整批隱藏。
另截 390px 手機版確認不破版。
